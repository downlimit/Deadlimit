...
This document preserves the useful results of the Ivy/Painter investigation,
records failed approaches alongside accepted evidence, and provides the current
continuation point for Deadlimit Shade research.
...
The follow-up experiment after `e865f17` attempted to extend
...
every live Deadlimit Hero/Retail Shader Instance. A fixed-camera Painter viewport
...
A fresh Painter viewport A/B on 2026-09-08 used the same explicit camera,
...
`NPR Specular Material Tint` and `Retail Environment F0`. Painter viewport validation captured
...
viewport was captured from the Painter viewport. Direct diffuse, direct specular,
...
Visual follow-up: Painter viewport validation verified neutral NPR Composite, material Shaded,
...
the environment integration as a complete path using those inputs. No additional artistic coefficients or viewport PASS were introduced in this step.
...
The visual gate remained pending because the automated GUI capture targeted the
wrong foreground application rather than Painter, so no valid Painter viewport
evidence was recorded. Captured mode was disabled in the open project; Material /
Retail and Shaded were restored, and the experimental SPP was not saved. This
iteration therefore did not establish visual progress.
...
is enabled in the open project, Material / Retail and Shaded selected. This is a visual pass for atlas connectivity only; full CSDK
parity remains unproved and the documented BRDF approximations still apply.
The project was not saved over the existing SPP during this check.
...
and prior visual status are unchanged. No full visual PASS was claimed from this source-only transfer.
...
mutation or visual PASS. At the time of this record, these post-checkpoint changes were not yet part of the accepted baseline.
...
compiled by Painter. Real Painter viewport frames are local under
