...
Assert-True $backend.Contains('list-pattern') 'Retail inspection must support bounded VPK resource discovery.'
Assert-True $backend.Contains('CreateLatLongFromCubemapFaces') 'Retail environment extraction must convert Source 2 cubemap faces to a Painter-compatible lat-long image.'
Assert-True $backend.Contains('compiledSha256') 'Retail environment manifests must preserve compiled-resource identity.'
