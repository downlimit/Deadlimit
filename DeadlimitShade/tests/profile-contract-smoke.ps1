...
foreach ($profile in $profiles) {
    ...
    Assert-True ([bool] $profile.rim.enabled) 'Ivy character preset must enable the recovered rim.'
    Assert-Near ([double] $profile.rim.cutoff) 1.0 0.0000001 'Ivy rim cutoff must retain the runtime-backed value.'
    Assert-Near ([double] $profile.rim.sharpness) 0.01 0.0000001 'Ivy rim sharpness must retain the runtime-backed value.'
    Assert-Near ([double] $profile.rim.strength) 0.3 0.0000001 'Ivy rim strength must retain the runtime-backed value.'
    Assert-Near ([double] $profile.rim.upRamp[0]) 0.0 0.0000001 'Ivy rim up-ramp minimum must retain the runtime-backed value.'
    Assert-Near ([double] $profile.rim.upRamp[1]) 1.0 0.0000001 'Ivy rim up-ramp maximum must retain the runtime-backed value.'
    Assert-True ($profile.rim.runtimeSource -eq 'reduced-csdk-asset-browser') 'Rim defaults must preserve Reduced-CSDK provenance.'
    Assert-True ($profile.rim.evidence -eq 'confirmed-pipeline-runtime') 'Rim defaults must retain runtime evidence.'
...
Assert-True ($heroShader.Contains('"Deadlimit Rim Mask": 16')) 'Hero shader is missing the resolved author/retail rim-mask diagnostic.'
Assert-True ($heroShader.Contains('//: param auto channel_user0')) 'Hero shader must expose Painter User0 as a paintable rim mask.'
Assert-True ($heroShader.Contains('uniform SamplerSparse dl_rim_mask_tex;')) 'Rim mask must use Painter sparse channel sampling.'
Assert-True ($heroShader.Contains('//: param auto channel_user1')) 'Hero shader must expose Painter User1 as paintable Artistic AO.'
Assert-True ($heroShader.Contains('uniform SamplerSparse dl_artistic_ao_tex;')) 'Artistic AO must use Painter sparse channel sampling.'
Assert-True ($heroShader.Contains('value.r + fallbackValue * (1.0 - value.g)')) 'Paintable rim mask must fall back through the sparse residency lane.'
Assert-True ($heroShader.Contains('dlDebugOutput(vec3(ambientOcclusion));')) 'Ambient Occlusion debug must show the resolved Artistic AO value.'
Assert-True ($heroShader.Contains('"group": "Deadlimit Rim Light"')) 'Shader Settings must expose a dedicated Deadlimit Rim Light group.'
