...
Flip Polygons and Invert Normals are on by default. The complete mesh and all map faces are generated and stitched first. Flip Polygons then performs one native Editable Poly element flip over the finished face set, including matching UV/map-face winding. Invert Normals adds a temporary Edit Normals modifier, makes every normal explicit, reverses them, and collapses the result into the base Editable Poly. Copy Skin, when enabled, runs after this bake so the output stack contains Skin without a leftover Edit Normals modifier. Together the default operations produce inward-facing geometry whose explicit normals point back toward the original surface, matching the intended expanded-backface outline. Either operation can be disabled independently.
