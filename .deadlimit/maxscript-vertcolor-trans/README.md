...
Smoothing groups, material IDs, and edge visibility are restored for original faces. The generated topology lives in a dedicated DEADLIMIT INNER LINEART Edit Poly modifier. The pass can be enabled or disabled without changing the source Edit Poly or base object.

When an Edit Poly is active inside a stack, the pass is inserted directly above it. Modifiers above and below it, including Skin, remain in place. For a stacked object the intended source Edit Poly must be active. A bare Editable Poly receives the same dedicated pass. A bare Editable Mesh is converted to Editable Poly first while retaining its physical edge selection. Conversion and topology creation use one Undo step.
