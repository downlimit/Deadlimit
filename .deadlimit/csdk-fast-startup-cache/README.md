...
The tool reads `CsdkRoot` from the legacy compatibility path `%LOCALAPPDATA%\Deadlimit\settings.json`. Open Deadlimit Manager Settings, configure **Reduced CSDK12**, and press **SAVE** before the first run.
...
Скрипт берёт путь `Reduced CSDK12` из настроек Deadlimit Manager. Сами настройки по-прежнему хранятся в `%LOCALAPPDATA%\Deadlimit\settings.json` ради совместимости. Перед запуском закройте все окна CSDK. Предыдущий рабочий кеш сохраняется до успешного завершения и восстанавливается при ошибке.
