...
2. при необходимости создаёт пустой content-партнёр `content\citadel\addons\luaunlocker` для runtime-папки `game\citadel\addons\luaunlocker`, чтобы Source 2 AssetSystem мог корректно следить за search path;
3. отказывается работать, пока запущены окна этой CSDK;
4. сохраняет копию предыдущего кеша;
5. запускает Citadel с `-savereadonlyassets`;
6. ждёт полной инициализации AssetSystem и нового кеша;
7. закрывает только созданный им временный процесс;
8. восстанавливает предыдущий кеш при ошибке.
