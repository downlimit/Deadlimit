    ...
    private static readonly IReadOnlyDictionary<string, string> ExactRewrites =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Save this project's selected hero and Release ID.\n\nAfter a successful save, hero selection is locked again to protect the project from accidental changes."] =
                "**SAVE PROJECT** saves the selected hero and **Release ID**.\n\nUse it after choosing the project's hero or changing its release slot. Model and texture files are tracked automatically.\n\nAfter saving, hero selection is locked again to prevent accidental changes.",
            ["Сохранить выбранного героя и Release ID проекта.\n\nПосле успешного сохранения выбор героя снова блокируется, чтобы защитить проект от случайной смены."] =
                "**СОХРАНИТЬ ПРОЕКТ** сохранит выбранного героя и **Release ID**.\n\nИспользуйте после выбора героя проекта или изменения его слота. Файлы моделей и текстур отслеживаются автоматически.\n\nПосле сохранения выбор героя снова блокируется от случайной смены.",
            ...
            ["Run the optional full CSDK setup from the current installation guide.\n\nDeadlimit stages the required Deadlock depots, extracts the downloaded VPK as-is, removes the temporary pak01 VPK set, then re-applies Reduced CSDK. A normal repeat skips all downloads when the same guide setup is already complete.\n\nHold **SHIFT** to force a complete repair download. DepotDownloader may open a console for Steam QR authentication.\n\nThe configured Deadlock client folder is only validated and is **never modified**."] =
                "Set up the extra Deadlock files that CSDK needs for a complete working environment.\n\nUse this if the normal Reduced CSDK installation is not enough for your work. Completed fine-tuning is detected and a normal repeat does not download or change files.\n\nHold **SHIFT** to force a complete repair download. Files are staged before they are applied. Steam may ask you to sign in with a QR code in a separate window. Your installed **Deadlock** game files are **never changed**.",
