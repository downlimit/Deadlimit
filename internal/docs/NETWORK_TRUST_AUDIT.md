...
| --- | --- | --- | --- | --- |
| Developer repository updater | `https://github.com/downlimit/Deadlimit.git`, branch `main` | Fast-forward of a developer Git checkout, then local build | HTTPS plus Git object integrity; no signed release boundary | Developer channel only. |
| Artist installer/updater | GitHub Release tag `latest-main`; exact package, checksum, installer, and metadata asset names | The one-file installer places the package under the user profile and creates shortcuts; the shared updater preserves local `UserData` | HTTPS, source-host allowlist, SHA-256 before extraction and activation, contained ZIP extraction, file manifest, local `Backup` rollback | This is the single artist channel. Successful `main` CI replaces the rolling assets together and publishes metadata last. |
...
- The updater shipped inside the artist package verifies the next ZIP before
...
- CSDK, DepotDownloader, and DeadlockTools install/update actions require an
  ...
  is identical in Git and artist installations.
