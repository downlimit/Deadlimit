...
- Deadlimit Manager desktop launcher/updater/icon workflow.
...
- Deadlimit Manager-managed VMAT texture slots reconcile on every PREPARE;
- user-authored/custom VMAT settings outside Deadlimit Manager-managed texture reconciliation remain preserved.
...
- if Release ID changes, an old slot is removed only when its file still matches Deadlimit Manager's recorded hash;
- completion dialog offers OK / launch Deadlock when the game is not running;
- if Deadlock is already running, Deadlimit Manager does not force a restart while hot-reload behavior is still unproven;
...
Final acceptance for Stage 3 requires one run after Deadlimit Manager Updater that proves the **current** implementation:

```text
Deadlimit Manager Updater
→ BUILD & TEST
...
→ final Deadlimit Manager dialog appears
...
Goal: improve the artist-facing Deadlimit Manager application now that the functional pipeline is closed.
