...
`LAUNCH GAME / ЗАПУСК ИГРЫ` launches Deadlock through the installed Steam client using app id `1422450`, with the Steam URI as a fallback. Holding SHIFT while clicking the game-launch button also copies `cl_lock_camera true` to the Windows clipboard for visual testing.
