...
- `SHIFT + LAUNCH CSDK`: run the normal preserving PREPARE, start LIVE SYNC, and launch CSDK. Repeating the gesture stops LIVE SYNC without launching another CSDK instance.

LIVE SYNC watches supported root textures, artist DMX files, and matching `*_vertexcolor.fbx` sidecars. The extended DMX/FBX pair debounce applies only when a renderable mesh has a faceSet whose assigned material name or `mtlName` contains `vertexcolor` (case-insensitive), embedded color is unavailable, and the external sidecar is missing or stale. A stale sidecar beside an unrelated DMX does not delay live synchronization. A sidecar created after the base DMX debounce is read as its own event: the prepared DMX target is refreshed from the artist DMX and Vertex Color is validated/applied immediately. A rejected required sidecar marks the session as requiring normal PREPARE and reports the reason.

When a LIVE SYNC edit changes project structure or material references, LIVE SYNC now runs the preserving PREPARE transaction automatically, refreshes its watcher baseline, and keeps the current CSDK instance open. Changes that arrive while that transaction is running, or alter the root-source snapshot during the transaction, request one more pass, so the replacement baseline cannot silently accept a newer root file than the content copied into CSDK. A failed automatic transaction keeps the last good prepared content and reports that manual PREPARE is required for recovery.
