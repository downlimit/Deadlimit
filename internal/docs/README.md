...
3. **Iterate in game** — after authoring is established, use one `BUILD & TEST` action to prepare changes, compile, restore required model post-processing, package the VPK, and deploy it directly to retail Deadlock addons. If retail Deadlock is currently running, Deadlimit Manager must close it first because the loaded VPK is file-locked by the game.
