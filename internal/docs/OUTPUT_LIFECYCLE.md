# Deadlimit Manager — content/game output lifecycle
...
### Deadlimit Manager contract

Deadlimit Manager treats the two trees asymmetrically:
...
The addon name comes from the manifest's permanent `AddonId`. Before either transaction touches CSDK output, Deadlimit Manager verifies `.deadlimit-addon-owner.json` in the addon content root against the project's permanent `ProjectId`. An unreadable record, a foreign owner, or a pre-existing unclaimed folder stops the operation before recursive cleanup. Legacy projects may adopt their existing addon only when their stored source/compiled VMDL path proves the relationship.
...
Deadlimit Manager now has two deliberately different transactions.
...
For Deadlimit Manager-managed CUSTOM materials, the project-root PNG set remains authoritative.
...
- Deadlimit Manager-managed V4 VMATs reconcile texture assignments on every preparation, including add/remove behavior;
