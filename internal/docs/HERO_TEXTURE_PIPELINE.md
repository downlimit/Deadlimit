...
- A supported root image whose basename uniquely matches an extracted retail texture targets that retail texture's original Source 2 resource path during PREPARE.
- Ambiguous basenames fail closed with an explicit error. Deadlimit never guesses between multiple retail targets.
...
4. Resolve project-root images against that index by strict unique basename matching.
...
7. Add smoke coverage for enabled/disabled extraction, unique mapping, ambiguous refusal, alpha-bearing sources, deletion cleanup, and custom-material non-regression.
