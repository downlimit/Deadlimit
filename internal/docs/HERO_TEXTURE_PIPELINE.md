...
- LIVE SYNC routes edits to an existing retail replacement back to the same retail resource path; ordinary project/custom-material textures retain their existing addon texture target.
...
6. LIVE SYNC routing for existing root retail replacements — **implemented**.
7. Dedicated Windows CI for dependency/wiring and override-resolution smokes — **implemented and running on `main`**.
