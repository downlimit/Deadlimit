...
$renderAnchor = '    private static readonly Regex RenderMeshRegex = new('
$renderIndex = $text.IndexOf($renderAnchor, [StringComparison]::Ordinal)
if ($renderIndex -lt 0) { throw 'RenderMeshRegex declaration was not found.' }
$afterRender = $text.IndexOf(";`n", $renderIndex, [StringComparison]::Ordinal)
if ($afterRender -lt 0) { throw 'RenderMeshRegex declaration terminator was not found.' }
$afterRender += 2
...
'@
$text = $text.Insert($afterRender, $regexBlock)
